...
public class ClusterResourceMonitor {
	...
	private void clusterMonitor() {
		try {
			storageEngine.writeDataPointWithLock(Point.newBuilder().setDbName(DB).setMeasurementName("cluster")
					.setTimestamp(System.currentTimeMillis())
					.addTags(Tag.newBuilder().setTagKey("type").setTagValue("controller").build())
					.addValue(connector.getClusterSize()).addFp(false).build(), true);
		} catch (Exception e) {
