...
public class SidewinderSchemaFactory implements SchemaFactory {
	...
	public SidewinderSchemaFactory() {
		...
		try {
			this.engine.configure(new HashMap<>());
			this.engine.connect();
			for (int i = 1; i < 10; i++) {
				engine.writeDataPoint("test", new DataPoint("test", "cpu", "value", Arrays.asList("host"),
						System.currentTimeMillis() - 1000 + i * 10, 2.2 * i));
			}
		} catch (IOException e) {
	...
	public static class SidewinderConverter extends ConverterRule {

		public SidewinderConverter(Class<? extends RelNode> clazz, RelTrait in, RelTrait out, String description) {
			super(clazz, in, out, description);
		}

		@Override
		public RelNode convert(RelNode rel) {
			System.out.println("Capture converter:" + rel);
			return rel;
		}

	}

	public static class MeasurementTable extends AbstractTable implements FilterableTable {
	...
	public static class SidewinderDatabaseSchema extends AbstractSchema {
