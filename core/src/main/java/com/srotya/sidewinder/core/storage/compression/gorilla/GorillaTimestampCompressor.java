...
/**
 * Implements a slightly modified version of the time series compression as
 * described in the Facebook's Gorilla Paper.
 *
...
public class GorillaTimestampCompressor {
	...
	public GorillaTimestampCompressor(long timestamp, BitOutput output) throws IOException {
		this(timestamp, output, new LastValuePredictor());
	}

	public GorillaTimestampCompressor(long timestamp, BitOutput output, Predictor predictor) throws IOException {
		blockTimestamp = timestamp;
		out = output;
		addHeader(timestamp);
	}

	private void addHeader(long timestamp) throws IOException {
	 ...
	 *
	 * @param timestamp
	 *            Timestamp which is inside the allowed time block (default 24 hours
	 *            with millisecond precision)
	 * @throws IOException
	 */
	public void addValue(long timestamp) throws IOException {
		if (storedTimestamp == 0) {
			writeFirst(timestamp);
		} else {
			compressTimestamp(timestamp);
		}
	}

	private void writeFirst(long timestamp) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void compressTimestamp(long timestamp) throws IOException {
