...
public class TestPredicates {
	...
	public void testAndPredicate() {
		...
		Predicate predicate = new AndPredicate(Arrays.asList(new EqualsPredicate(ts)));
		assertTrue(predicate.apply(ts));
		predicate = new AndPredicate(
				Arrays.asList(new LessThanEqualsPredicate(ts + 1), new GreaterThanEqualsPredicate(ts - 1)));
		assertTrue(predicate.apply(ts));
		assertTrue(!predicate.apply(ts + 4));
	}
	...
	public void testBetweenPredicate() {
		...
		assertTrue(predicate.apply(ts));
		assertTrue(!predicate.apply(ts + 2));
	}
	...
	public void testGreaterThanEqualsPredicate() {
		...
		assertTrue(predicate.apply(ts));
		assertTrue(predicate.apply(ts - 1));
		assertTrue(!predicate.apply(ts - 2));
	}
